...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    // Use Chrome on macOS, system/puppeteer chromium on Linux, default on others
    ...
    if (process.platform === 'darwin') {
      ...
      for (const c of candidates) {
        try { require('fs').accessSync(c, require('fs').constants.X_OK); launchOptions.executablePath = c; break; } catch {}
      }
    }

"""scripts/p8_merge_external_data.py — 외부 이적 데이터를 기존 transfer_dataset에 병합.

크롤링 완료 후 이 스크립트를 실행한다.

실행 순서:
  1. python scripts/p8_championship_crawler.py   (크롤링, ~60분)
  2. python scripts/p8_merge_external_data.py    (병합, 수초)
...
EXT_PATH = ROOT / "data" / "raw" / "p8_championship_transfers.parquet"
...
log = logging.getLogger("p8_merge")


def main() -> None:
    log.info("=" * 60)
    log.info("🔀 P8 외부 이적 데이터 병합")
    log.info("=" * 60)

    # 외부 데이터 확인
    if not EXT_PATH.exists():
        log.error(f"❌ 외부 데이터 없음: {EXT_PATH}")
        log.error("   먼저 python scripts/p8_championship_crawler.py 를 실행하세요.")
        sys.exit(1)

    df_ext = pd.read_parquet(EXT_PATH)
    log.info(f"📂 외부 데이터: {len(df_ext)}건 (Championship→EPL)")
    ...
    log.info(f"   league_level_diff 분포:")
    if "league_level_diff" in df_merged.columns:
        log.info(f"   {df_merged['league_level_diff'].value_counts().to_string()}")
